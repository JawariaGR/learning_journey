// QUESTION NO 27 (CONVERT YOUR IF AND ELSE CHAIN TO HANDLE THREE COLOR "RED","GREEN","YELLOW")

// RED ALIEN COLOR
let ALIEN_Color: string = "red";
if (ALIEN_Color === "red") {
  console.log("I guess you don't know i'm his Unbreakable Sword");
} else if (ALIEN_Color === "yellow") {
  console.log(
    "Do You know which revenge is most powerful ? it's deying that person from what he really love"
  );
} else if (ALIEN_Color === "green") {
  console.log("when you can't control yourself,count to three");
}

//YELLOW ALIEN COLOR
let alien_colors: string = "yellow";
if (alien_colors === "red") {
  console.log("The person I like the most is myself");
} else if (alien_colors === "yellow") {
  console.log(
    "What the shadow witch had stolen from them, was not their ture faces but their courge to find happiness"
  );
} else if (alien_colors === "green") {
  console.log(
    "If you want to make around you happy you have to find your own happiness first"
  );
}

//GREEN ALIEN COLOR
let ALIEN_COLORS: string = "green";
if (ALIEN_COLORS === "yellow") {
  console.log("you don't need to try so hard. just relax and be yourself");
} else if (ALIEN_COLORS === "red") {
  console.log("you can't get out once you're trapped in the past");
} else if (ALIEN_COLORS === "green") {
  console.log("Is being different something to be afraid of ?");
}
