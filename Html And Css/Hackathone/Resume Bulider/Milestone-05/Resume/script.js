...
let edit_btn = document.getElementById("edi-btn");
...
let shar_btn = document.getElementById("Share-btn");
let ancer = document.getElementById("ancer");
let client_name;
if (myname) {
    client_name = myname.toLocaleLowerCase().replace(/\s+/g, "-");
}
else {
    client_name = "user";
}
let baseUrl = "http://127.0.0.1:5501/Resume/index.html";
let clientUrl = `${baseUrl}?/${myname}`;
shar_btn?.addEventListener("click", () => {
    ancer?.setAttribute("href", clientUrl);
});
